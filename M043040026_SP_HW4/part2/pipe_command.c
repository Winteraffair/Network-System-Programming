	...
	int count;
  	...
  	switch (pipe_argv_index) {
    	...
    	default:	/* Pipe in the middle of argv array.  See pipe_present(). */
			 ...
			 *
       		 * Fill in code. */
		for(count = 0;count < pipe_argv_index;count++){
		    left_argv = (char **)realloc(left_argv,sizeof(char*)*(count+1));
		    left_argv[count]=(char *)malloc(sizeof(char)*(strlen(myArgv[count])+1));
		    strcpy(left_argv[count],myArgv[count]);
		}
		left_argv = (char **)realloc(left_argv,sizeof(char*)*(count+1));
		left_argv[count]=NULL;
		
		for(count = pipe_argv_index+1 ;myArgv[count] != NULL;count++){
		    right_argv = (char **)realloc(right_argv,sizeof(char*)*(count-pipe_argv_index));
		    right_argv[count - (pipe_argv_index+1) ]=(char *)malloc(sizeof(char)*(strlen(myArgv[count])+1));
		    strcpy(right_argv[count - (pipe_argv_index+1)],myArgv[count]);
		}
		
		right_argv = (char **)realloc(right_argv,sizeof(char*)*(count-pipe_argv_index));
		right_argv[count]=NULL;
