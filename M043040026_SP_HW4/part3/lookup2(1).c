...
int lookup(Dictrec * sought, const char * resource) {
	...
	if (first_time) { /* set up index */
		 ...
		 * Fill in code. */
                for(i=0;i<numrec;i++){
                  fread(table[i].word,sizeof(dr.word),1,in);
                  table[i].off=ftell(in);
                  fseek(in,sizeof(dr.text),SEEK_CUR);
	...
	if (found) {
		...
		fread(sought->text,sizeof(dr.text),1,in);
		return FOUND;
	}
	return NOTFOUND;
}
